...
function loadCalendar(calendarList) {
	...
	for (el of calendarList) {
		...
		wrapList.innerHTML += `<li class="${el.type}"><p><b>${el.type == 'event' ? `<a href="events.html#${el.link}" class="anchorCalendar">${el.title}</a>` : el.title + "<br>"}</b>${info}</p><p class="timeAndLocation">Time: ${el.tid}<br>Location: ${el.sted}</p></li>`;
