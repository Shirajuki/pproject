const articles = [
  {
    ...
    id: "welcome_new_members", //makes it easier to create short-forms to use for id and anchor links //other possible solution: using split to remove whitespace and characters that are not letter -> get first 6 letters to make id/anchor link
    date: "2019-09-15",
...
const article_list = document.getElementById("list_with_articles");
...
window.addEventListener('load', () => {
  ...
  for (let i = 0; i < articles.length; i++) {
    ...
    let sec_element = `<section class="newsarticle"><a id="${articles[i].id}" class="anchor"></a><div class="news_heading"><h2>${articles[i].title}</h2><span class="span_date">${new Date(articles[i].date).toLocaleDateString("nb-NO",{year: 'numeric', month: '2-digit', day: '2-digit'})}</span></div><p>${articles[i].text}</p></section><hr>`;
    li_element.innerHTML += sec_element;  //fills the li-element with content
    article_list.prepend(li_element); //writes to html-element with id list_with_articles, prepends the node to the start of the list
  }
  ...
  for (let i = 0; i < newest.length; i++) {
    let li_nav_element = document.createElement("li");
    let li_nav_anchor = `<a href="#${newest[i].id}">${newest[i].title}</a>`;
    li_nav_element.innerHTML = li_nav_anchor;
